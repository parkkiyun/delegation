...
with col2:
    # 버튼 클릭 시 웹페이지 열기
    if st.button("사용법 보기"):
        st.markdown("[사용법 안내 페이지](https://blog.naver.com/basara3447/223646533150)", unsafe_allow_html=True)
...
if 'step' in st.session_state and st.session_state['step'] == 2:
    ...
    if uploaded_file is not None:
        ...
        def unmerge_excel_cells(file_path):
            ...
            # 변경된 내용을 임시 파일로 저장
            temp_file = "/tmp/unmerged.xlsx"
            wb.save(temp_file)
            return temp_file

        try:
            # 병합된 셀 분리 처리
            unmerged_file = unmerge_excel_cells(uploaded_file)
            ...
            if missing_columns:
                ...
                data = data[['번호', '성명', '출���구분', '사유', '결석시작일', '결석종료일', '결석일수', '결석확인일']]
